...
public class ModePickerRotateLayout extends ViewGroup implements Rotatable {
    private static final LogHelper.Tag TAG = new LogHelper.Tag(ModePickerRotateLayout.class.getSimpleName());
    private View mChild;
    private int mOrientation;

    public ModePickerRotateLayout(Context context, AttributeSet attributeSet) {
    ...
    public void setOrientation(int i, boolean z) {
        LogHelper.m23d(TAG, "setOrientation(" + i + ", " + z + ") mOrientation=" + this.mOrientation);
        int i2 = ((i - 270) + 360) % 360;
        if (this.mOrientation != i2) {
            this.mOrientation = i2;
            CameraUtil.setOrientation(this.mChild, this.mOrientation, z);
            requestLayout();
        }
    }
}
