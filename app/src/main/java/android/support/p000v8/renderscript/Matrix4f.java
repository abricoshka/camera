...
public class Matrix4f {
    ...
    public void loadRotate(float f, float f2, float f3, float f4) {
        ...
        float fCos = (float) Math.cos(f5);
        float fSin = (float) Math.sin(f5);
        ...
        float f12 = f3 * fSin;
        float f13 = fSin * f4;
        this.mMat[0] = (f2 * f2 * f7) + fCos;
        this.mMat[4] = (f8 * f7) - f13;
        this.mMat[8] = (f10 * f7) + f12;
        this.mMat[1] = f13 + (f8 * f7);
        this.mMat[5] = (f3 * f3 * f7) + fCos;
        this.mMat[9] = (f9 * f7) - f11;
        this.mMat[2] = (f10 * f7) - f12;
        this.mMat[6] = (f9 * f7) + f11;
        this.mMat[10] = (f7 * f4 * f4) + fCos;
    }
    ...
    private float computeCofactor(int i, int i2) {
        ...
        int i7 = (i2 + 2) % 4;
        int i8 = (i2 + 3) % 4;
        float f = (this.mMat[i3 + (i8 * 4)] * ((this.mMat[(i6 * 4) + i4] * this.mMat[(i7 * 4) + i5]) - (this.mMat[i4 + (i7 * 4)] * this.mMat[i5 + (i6 * 4)]))) + ((this.mMat[(i6 * 4) + i3] * ((this.mMat[(i7 * 4) + i4] * this.mMat[(i8 * 4) + i5]) - (this.mMat[(i8 * 4) + i4] * this.mMat[(i7 * 4) + i5]))) - (this.mMat[(i7 * 4) + i3] * ((this.mMat[(i6 * 4) + i4] * this.mMat[(i8 * 4) + i5]) - (this.mMat[(i8 * 4) + i4] * this.mMat[(i6 * 4) + i5]))));
        return ((i + i2) & 1) != 0 ? -f : f;
    }
        ...
        while (true) {
            ...
            if (i2 < 3) {
                for (int i3 = i2 + 1; i3 < 4; i3++) {
                    float f = this.mMat[(i2 * 4) + i3];
                    this.mMat[(i2 * 4) + i3] = this.mMat[(i3 * 4) + i2];
                    this.mMat[(i3 * 4) + i2] = f;
                }
                i = i2 + 1;
            } else {
