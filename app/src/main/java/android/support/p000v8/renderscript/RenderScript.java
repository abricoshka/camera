...
public class RenderScript {
    ...
    private static boolean setupNative(int i, Context context) throws PackageManager.NameNotFoundException {
        ...
        if (mBlackList.length() <= 0 || !mBlackList.contains('(' + Build.MANUFACTURER + ':' + Build.PRODUCT + ':' + Build.MODEL + ')')) {
            return true;
        }
        sNative = 0;
        return false;
        ...
        if (z) {
            ...
            if (!this.mIncLoaded) {
                try {
                    ...
                    if (!nIncLoadSO(SUPPORT_LIB_API, this.mNativeLibDir + "/libRSSupport.so")) {
    ...
    private static RenderScript internalCreate(Context context, int i, ContextType contextType, int i2) {
        ...
        synchronized (lock) {
            if (!sInitialized) {
                ...
                try {
                    if (Build.VERSION.SDK_INT < SUPPORT_LIB_API && renderScript.mNativeLibDir != null) {
